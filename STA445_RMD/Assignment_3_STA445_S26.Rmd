...
duniform(x = 1, a = 2, b = 5) 
duniform(x = 3, a = 2, b = 5)   
duniform(x = 6, a = 2, b = 5)   
```
...
```{r}
duniform_ifelse <- function(x, a, b) 
{
...
```{r}
data.frame(x = seq(-1, 12, by = .001)) %>%
  mutate(y = duniform_ifelse(x, 4, 8)) %>%
  ggplot(aes(x = x, y = y)) +
  geom_step()
```

```{r}
microbenchmark::microbenchmark( duniform_ifelse( seq(-4,12,by=.0001), 4, 8), times=100)
```
The if else version was definitely faster and easier to write sinc ethere was no for loop. The if else version ran in 3.3 ms and the for loop version 9.5 ms.
